...
>
> （倒排索引将词（或术语）映射到出现在这些词中的文档列表，与传统正排索引相反，正排索引将文档映射到其中包含的词列表。）
>
> 假设我们的文档集包含三个文档：
>
> - 文档1：`"This is a test"`
> - 文档2：`"This test is a sample"`
> - 文档3：`"Test this sample"`
>
> 倒排索引将生成如下结构：
>
> ```
...
> ### 什么是函数式编程语言？
>
> **函数式编程语言**是一种通过“函数”来处理和组织代码的编程语言。以下是其中的一些核心思想：
>
> 1. **函数就是“普通的东西”**：
>    - 在函数式编程中，函数就像变量一样，可以随意使用。
>    - 你可以把函数传递给其他函数，或者让函数返回另一个函数（像变量返回一样）。
> 2. **不改变数据**：
>    - 一旦创建了某个数据，这些数据不会被改变。而是使用原始数据来创建新的数据。这使得代码更容易理解和调试。
> 3. **没有副作用**：
>    - 函数的运行不会影响系统的其他部分，每次调用同样的函数，输入一样，结果一定一样。
>
> ### 对比一下：
>
> #### 面向对象编程 (OOP)
...
>         myCat.setName("Garfield");  // 修改了猫的名字
>         System.out.println(myCat.getName()); // Garfield
>     }
> }
> ```
>
...
> -- 使用这个函数
> main = do
>     let name = catName "Whiskers"
>     let newName = catName "Garfield"
>     putStrLn newName  -- Garfield
> ```
>
> 在这里，我们没有“修改”名字，而是**创造**了一个新的名字。
>
> ### 简单例子
>
> 假如你有一个列表 [1, 2, 3, 4, 5]，你想把每一个元素乘以 2，然后把结果加起来。
>
> #### 在面向对象的风格中：
>
> ```java
...
> ```
>
> #### 在函数式编程的风格中（假设使用 Java 8 的 Stream API）：
>
> ```java
...
> ```
>
> 在这段函数式风格的代码中，我们没有“改变”原始数据，而是通过函数（`map` 和 `sum`）来描述我们想要的结果。
>
> 
...
func main() {
  ...
  for i := 0; i < 10; i ++ {
    // 匿名函数，创建共 10 个线程
    go func() {
      vote := requestVote() // 一个内部sleep随机时间，最后返回true/false的函数，模拟投票
      if vote {
        count++
      }
      finished++
...
> 当你创建并启动一个 goroutine 时，它会在后台并发执行。这意味着循环的每次迭代都会立即启动一个新的 goroutine，而无需等待前一个 goroutine 完成。通过**并发**，多个任务将同时进行（在 CPU 的调度下），而不是串行地一个接一个执行。想到goroutine就不要想着串行应该想着并发（即不用等别人）
>
> **外部 `for` 循环会快速启动 10 个 goroutine，每个 goroutine 并行运行，并互不影响地去执行投票任务。**这也说明可以用for循环快速启动goroutine
...
**利用defer语言，一般在声明加锁后，立即defer声明推迟解锁** 
...
func main() {
  ...
  finished := 0
  var mu sync.Mutex
  // 条件变量 和 指定的 lock 绑定，使用互斥锁mu创建一个条件变量cond
  ...
  for i := 0; i < 10; i ++ {
    ...
    go func() {
      ...
      //cond.Broadcast()：唤醒所有在 cond.Wait() 上等待的 goroutine。只有在一定条件满足（例如这里的投票数达到）时才会这么做。
      cond.Broadcast()
    } () 
  }
...
> [Go RPC ](https://www.jianshu.com/p/5ade587dbc58)<= 该小节简单讲解了RPC，这里推荐这篇文章。或者可以去了解下**gRPC**。

RPC(Remote Produce Call)远程过程调用。RPC是一个计算机通信协议，允许调用不同进程空间的程序。举例：电脑上用java springboot写的服务器端代码，可以调用电脑上的另一个用c语言写的打印helloworld到文件中的程序
